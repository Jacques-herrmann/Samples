...
export default class Debug extends EventEmitter2 {
  ...
  debugLight (light, name) {
    ...
    this.folders.lights.forEach((folder, f) => {
      const configKeys = Object.keys(config.lights[name])
      if (configKeys.includes('color')) {
        folder.addInput(config.lights[name], 'color').on('change', () => {
          light.color.set(config.lights[name].color)
        })
        configKeys.splice(configKeys.indexOf('color'), 1)
      }
      const basicKeys = ['position', 'color', 'intensity', 'distance', 'angle']
      configKeys.forEach(k => {
